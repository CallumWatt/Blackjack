...
void clearScreen() {
    system("clear");  // Clears the screen in Linux
}

void wait(int milliseconds) {
    std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));  // Replaces Sleep()
}
...
int drawCard() { // Draws a card for player
    ...
    if (cardGen <= 35 && cardGen > 4) { // If the card is anything else that isn't a 10 or an ace then creates random number between
        card = rand() % 9 + 2;
    }
